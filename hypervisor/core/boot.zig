...
pub fn bootCpuInit(cpu_allocator: std.mem.Allocator, dtb: [*]u8) !void {
    ...
    if (guest_arch == .aarch64) {
        ...
        const bootargs = try std.fmt.allocPrint(cpu_allocator, "console=hvc0 earlycon=sbi rootfstype=ramfs cma=32M no4lvl maxcpus={} lpj=50000 rcupdate.rcu_cpu_stall_suppress=1 rcutree.kthread_prio=1 unaligned_scalar_speed=fast video=1280x800 vt.global_cursor_default=0{s}", .{ guest_cpus, mitigations });
