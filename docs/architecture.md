...
process functions in Unix-like operating systems.
...
The hypervisor includes a pre-packaged implementation of a BuildRoot environment.
When compiling the project from source, the build system automatically
cross-compiles a complete RISC-V Linux kernel, a busybox-based userspace, and
necessary scripting environments for the initial Root VM. This ensures an
absolute guarantee of provenance by building all privileged guest code entirely
from source.
...
Diosix implements a subtree resource quota system to prevent Denial of Service
(DoS) scenarios and ensure fair resource distribution across the system. A VM's
quota defines the absolute maximum resources that it and its entire descendant
tree can consume.
...
RAM and page tables. Virtual Central Processing Unit (VCPU) limits define the
maximum number of VCPU cores allowed within a VM's specific subtree, while
scheduling priority limits define the maximum priority any VCPU in that subtree
can possess. Finally, Diosix enforces limits on the maximum child depth and the
...
Isolation is a fundamental pillar of the Diosix architecture. VMs are strictly
limited to communicating with their immediate parent and their direct children.
VMs that do not share a direct parent-child relationship are completely
isolated from one another and cannot interact or detect each other's existence.
...
The hypervisor enforces a cascading termination policy. If a VM terminates—either
through the standard `EXIT` call defined in the RISC-V SBI specification or a
fatal crash—the hypervisor recursively terminates all of its children and
...
Diosix explicitly categorizes address spaces to guarantee absolute clarity and
maintain separation of concerns throughout the codebase, debug outputs, and
documentation:

*  **Host Physical Address (HPA).** A physical memory address on the actual host
   hardware, such as physical DRAM, and memory-mapped peripheral registers
   (e.g., Core Local Interruptor (CLINT), Platform-Level Interrupt Controller
   (PLIC), or Universal Asynchronous Receiver-Transmitter (UART)).
...
These concepts are consistently referenced via their abbreviations (**HPA**,
**GPA**, and **GVA**) across the hypervisor's source code and diagnostics.
