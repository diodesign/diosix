...
fn main()
{
  ...
  println!("cargo:rerun-if-changed=src/platform/riscv32/{}/link.ld", target_machine);
  match fs::read_dir(format!("src/platform/riscv32/{}/asm", target_machine))
  {
        ...
        Ok(file) =>
        {
          ...
          if let Ok(metadata) = file.metadata()
          {
            if metadata.is_file() == true
            {
              ...
              assemble(&output_dir, file.path().to_str().unwrap(), &gnu_as, &gnu_ar);
            }
          }
        },
...
/* assemble
   ...
   => output_dir = where to assemble our code
      ...
      as_exec, ar_exec =
*/
fn assemble(output_dir: &String, path: &str, as_exec: &String, ar_exec: &String)
{
  ...
  Command::new(as_exec).arg("-fpic").arg("-o").arg(&object_file).arg(path)
